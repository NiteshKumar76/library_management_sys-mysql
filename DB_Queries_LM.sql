/* important Queries related to this project*/
...
SELECT issuestatus.Issue_book_name, customer.Customer_name FROM issuestatus 
INNER JOIN customer on issuestatus.Issued_customer = customer.Customer_Id;
...
WHERE Reg_date < '2025-01-01' AND Customer_Id NOT IN
(SELECT issued_customer FROM issuestatus);

# 7. Display the branch numbers and the total count of employees in each branch.
ALTER TABLE employee ADD COLUMN branch_id VARCHAR(10);
ALTER TABLE employee ADD CONSTRAINT FOREIGN KEY (branch_id)
...
# 8. Display the names of customers who have issued books in the month of may2023.
SELECT customer.Customer_name FROM customer
INNER JOIN issuestatus ON customer.Customer_Id = issuestatus.Issued_customer 
WHERE issuestatus.Issue_date >= '2025-05-01' AND issuestatus.Issue_date <= '2025-05-10';
...
SELECT Branch_id, COUNT(emp_id) FROM employee
GROUP BY Branch_id 
